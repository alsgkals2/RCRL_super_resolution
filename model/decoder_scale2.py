...
class Decoder_Id_RRDB(nn.Module):
    def __init__(self, num_in_ch, num_out_ch=3, scale=2, num_feat=64, num_block=10, num_grow_ch=32):
...
class Decoder_SR_RRDB(nn.Module):
    def __init__(self, num_in_ch, num_out_ch=3, scale=2, num_feat=64, num_block=23, num_grow_ch=32):
    ...
    def forward(self, x):
        ...
        body_feat = self.conv_body(self.body(feat))
        feat = feat + body_feat
        feat = self.lrelu(self.conv_up1(F.interpolate(feat, scale_factor=2, mode='nearest'))) #mhkim
#         feat = self.lrelu(self.conv_up2(F.interpolate(feat, scale_factor=2, mode='nearest'))) #mhkim
        out = self.conv_last(self.lrelu(self.conv_hr(feat)))
        return out
